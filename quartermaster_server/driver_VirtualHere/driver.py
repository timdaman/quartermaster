...
class VirtualHere(AbstractShareableUsbDevice):
    ...
    SERVICE_START_LOCKFILE = Path(tempfile.gettempdir()) / 'VirtualHereStartup.lock'
    CMD_TIMEOUT_SEC = 10
    ...
    def _vh_wrapper(self, *args, prefix=None):

        if prefix is None:
            command = [self.vhclient, *args, ]
        else:
            command = [*prefix, self.vhclient, *args]
    ...
    def vh_command(self, *args) -> str:
        try:
            ...
            # Making the lock timeout a little bigger than how long we allow commands to run
            with FileLock(self.SERVICE_START_LOCKFILE, timeout=self.CMD_TIMEOUT_SEC + 5) as locker:
    ...
    def start_client_service(self):
        try:
            if os.getuid() == 0:
                self._vh_wrapper('-a', '-n')
            else:
                self._vh_wrapper('-a', '-n', prefix=('sudo',))
        except subprocess.CalledProcessError as e:
            raise SystemError(f"Error starting VirtualHere client service: {e}: {e.output}")
